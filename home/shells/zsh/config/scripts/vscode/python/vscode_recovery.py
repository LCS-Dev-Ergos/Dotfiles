...
def _current_manifest_root(
    edition: str, stable_root: Path, insiders_root: Path
) -> Path:
...
def _build_alias_tasks(
    ...
    *,
    ...
    for request in requests:
        ...
        install_candidates = (
            resolved_stable_installs
            if install_root == stable_root
            else resolved_insiders_installs
        )
...
def plan_missing_extension_recovery(
    ...
    *,
    ...
    for decision in manifest_plan.decisions:
        ...
        manifest_root = _current_manifest_root(
            decision.edition.value, stable_root, insiders_root
        )
        ...
        if not (
            ...
            or is_preserved_missing_profile_decision(decision)
        ):
            continue
    ...
    for request in requests:
        candidate_pool = (
            stable_installs
            if request.install_root == stable_root
            else insiders_installs
        )
...
def _run_install_task(task: RecoveryInstallTask) -> bool:
    ...
    for install_spec in specs:
        ...
        try:
            ...
            # Installer binary missing or not executable; record a failed
            # install instead of crashing the recovery run.
            continue
...
def apply_missing_extension_recovery(
    ...
    *,
    ...
    for task in alias_tasks:
        root = (
            plan.stable_dir
            if plan.stable_dir in task.alias_path.parents
