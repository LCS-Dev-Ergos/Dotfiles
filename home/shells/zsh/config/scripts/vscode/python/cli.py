...
from vscode_sync_apply import (
    ExtensionMutationError,
    apply_extension_remove,
    apply_extension_setup,
)
...
def _sync_item_details(
    item: object, *, include_source: bool = False
) -> list[tuple[str, str]]:
...
def _print_extensions_overview(symlink_plan: object, manifest_plan: object) -> None:
    ...
    _print_metric(
        "Linked", f"{symlink_plan.linked_count}/{symlink_plan.expected_link_count}"
    )
    ...
    if manifest_plan.preserved_missing_profile_count:
        _print_metric(
            "Preserved profile drift", manifest_plan.preserved_missing_profile_count
        )
    ...
    detail_groups = (
        ...
        (
            "Wrong targets",
            _symlink_decisions_for(symlink_plan, SymlinkAction.WRONG_TARGET),
        ),
        (
    ...
    for title, decisions in detail_groups:
        ...
        for decision in decisions:
            ...
            if decision.action == SymlinkAction.WRONG_TARGET and decision.target_path:
                description = (
                    f"{decision.folder_name} -> {_pretty_path(decision.target_path)}"
                )
...
def _run_plan_cleanup(args: argparse.Namespace) -> int:
    ...
    _print_metric(
        "Respect references", _render_toggle(plan.respect_references), indent=4
    )
    _print_metric(
        "Prune stale refs", _render_toggle(plan.prune_stale_references), indent=4
    )
...
def _run_plan_links(args: argparse.Namespace) -> int:
    ...
    for decision in plan.decisions:
        print(
            f"  - {decision.folder_name}\t{decision.action.value}\treason={decision.reason}"
        )
    return 0
...
def _run_clean(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        ...
        if plan.respect_references:
            ...
            _print_metric(
                "Protected refs", len(plan.protected_reference_names), indent=4
            )
            if plan.stale_reference_names:
                ...
                _print_metric(
                    "Detected stale refs", len(plan.stale_reference_names), indent=4
                )
        else:
    ...
    if not args.yes:
        ...
        try:
            response = (
                input(f"{_style('Proceed with quarantine move? [y/N] ', _ANSI_YELLOW)}")
                .strip()
                .lower()
            )
        except EOFError:
            _print_notice(
                "error",
                "No confirmation input available; re-run with --yes to skip the prompt.",
            )
            return 1
...
def _resolve_shared_args(
    ...
    exclude_patterns = tuple(
        getattr(args, "exclude", None) or DEFAULT_EXTENSION_EXCLUDE_PATTERNS
    )
...
def _run_plan_manifests(args: argparse.Namespace) -> int:
    ...
    _print_metric(
        "Preserved profile drift", plan.preserved_missing_profile_count, indent=4
    )
    for decision in plan.decisions:
        if (
            decision.action == ManifestAction.KEEP
            and not is_preserved_missing_profile_decision(decision)
        ):
            continue
...
def _run_setup_extensions(args: argparse.Namespace) -> int:
    ...
    _print_metric(
        "Manifest updates", report.manifest_apply_report.updated_entries, indent=4
    )
    _print_metric(
        "Manifest removals", report.manifest_apply_report.removed_entries, indent=4
    )
    plan = plan_manifest_repairs(
        args.stable_dir,
        args.insiders_dir,
...
def _run_remove_extensions(args: argparse.Namespace) -> int:
    ...
    _print_metric(
        "Removed broken symlinks", report.removed_broken_symlink_count, indent=4
    )
    _print_metric("Skipped real directories", report.skipped_real_dir_count, indent=4)
    _print_metric(
        "Skipped foreign symlinks", report.skipped_foreign_symlink_count, indent=4
    )
...
def _run_sync_setup(args: argparse.Namespace) -> int:
    ...
    _print_metric(
        "Migrated unmanaged", report.extension_report.migrated_count, indent=4
    )
    _print_metric(
        "Removed stale", report.extension_report.removed_stale_symlink_count, indent=4
    )
    _print_metric(
        "Restored excluded",
        report.extension_report.restored_excluded_copy_count,
        indent=4,
    )
    _print_metric(
        "Skipped excluded",
        report.extension_report.skipped_excluded_symlink_count,
        indent=4,
    )
    _print_metric(
        "Manifest updates",
...
def _run_sync_remove(args: argparse.Namespace) -> int:
    ...
    _print_subsection("Extensions")
    _print_metric(
        "Restored root copies",
        report.extension_report.restored_root_copy_count,
        indent=4,
    )
    _print_metric(
        "Restored entry copies",
        report.extension_report.restored_entry_copy_count,
        indent=4,
    )
    _print_metric(
    ...
    _print_metric(
        "Skipped real dirs", report.extension_report.skipped_real_dir_count, indent=4
    )
    _print_metric(
        "Skipped foreign symlinks",
        report.extension_report.skipped_foreign_symlink_count,
        indent=4,
    )
    _print_metric("Failed paths", len(report.extension_report.failed_paths), indent=4)
    return (
        0
        if report.failed_count == 0 and not report.extension_report.failed_paths
        else 1
    )
...
def _run_update_extensions(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        ...
        if plan.skip_clean:
            ...
            _print_metric(
                "Duplicate groups", plan.cleanup_plan.duplicate_group_count, indent=4
            )
            _print_metric(
                "Current quarantine plan",
                plan.cleanup_plan.planned_deletion_count,
                indent=4,
            )
            _print_metric(
                "Manifest guard",
                "strict"
                if not plan.cleanup_plan.prune_stale_references
                else "prune stale refs",
                indent=4,
            )
        ...
        _print_metric(
            "Native checks", len(plan.native_excluded_extension_ids), indent=4
        )
    ...
    _print_metric(
        "Removed stale", report.setup_report.removed_stale_symlink_count, indent=4
    )
    _print_metric(
        "Restored excluded", report.setup_report.restored_excluded_copy_count, indent=4
    )
    _print_metric(
        "Manifest updates",
        report.setup_report.manifest_apply_report.updated_entries,
        indent=4,
    )
    _print_metric(
        "Manifest removals",
        report.setup_report.manifest_apply_report.removed_entries,
        indent=4,
    )
    print()
    ...
    _print_metric(
        "Wrong target", report.final_symlink_plan.wrong_target_count, indent=4
    )
    ...
    if (
        report.final_manifest_plan.update_count
        or report.final_manifest_plan.remove_count
    ):
        _print_metric(
            "Manifest updates", report.final_manifest_plan.update_count, indent=4
        )
        _print_metric(
            "Manifest removals", report.final_manifest_plan.remove_count, indent=4
        )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # A corrupt extensions.json must fail the command with a clear
        # message instead of a traceback; nothing has been mutated when
        # planning fails this way.
        _print_notice("error", f"Manifest error: {exc}")
        return 1


def _dispatch(parser: argparse.ArgumentParser, args: argparse.Namespace) -> int:
    """Route one parsed CLI invocation to its subcommand handler."""
