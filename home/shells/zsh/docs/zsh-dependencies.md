...
`home/shells/zsh/packages/zsh-dependencies.tsv` is the single source of truth. The
...
for python3, Homebrew or the distribution for the rest). `home/shells/zsh/default.nix`
...
mutating command it writes to `~/Dotfiles/home/shells/zsh` by default; set
...
```zsh
brew bundle --file ~/Dotfiles/home/shells/zsh/Brewfile
```
...
On the flake-managed host, `home/shells/zsh/default.nix` owns the documentation
toolchain: gawk from nixpkgs and shdoc, which nixpkgs does not package, built
by `home/shells/zsh/shdoc.nix` from the same checksum-pinned v1.4 release.
...
```zsh
sudo pacman -S --needed - < ~/Dotfiles/home/shells/zsh/packages/arch-zsh.txt
```
