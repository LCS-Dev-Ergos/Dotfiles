...
# ============================================================================ #
"""
Images Processing Script (canonical blog backend):
...
class ImagesProcessor:
    ...
    def __init__(self, config: Optional[Dict[str, str]] = None):
        ...
        # Create backup directory outside the repository tree.
        cache_root = os.environ.get("XDG_CACHE_HOME") or os.path.expanduser("~/.cache")
        self.backup_dir = os.path.join(cache_root, "blog", "backups", "images")
    ...
    @staticmethod
    def _load_configuration(config: Optional[Dict[str, str]]) -> Dict[str, str]:
    ...
    def process_image_reference(self, image_name: str) -> Tuple[Optional[str], bool]:
        """
        ...
        Returns:
            Tuple of (markdown_replacement, success_flag). The replacement is
            None on failure so the original Obsidian link stays in the post
            and the run remains repeatable after the problem is fixed.
        """
        # Validate image name.
    ...
    def process_file(self, file_path: str) -> bool:
        ...
        for image in images:
            ...
            # Replace occurrences only when processing succeeded; failed
            # references keep their original Obsidian link for a later retry.
            if markdown_replacement is None:
                continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert Obsidian image links to Hugo-compatible Markdown."
    )
    parser.add_argument(
        "posts_dir",
        nargs="?",
        default=None,
        help="Directory containing the Hugo posts (overrides BLOG_POSTS_DIR).",
    )
    parser.add_argument(
        "attachments_dir",
        nargs="?",
        default=None,
        help="Obsidian attachments directory (overrides OBSIDIAN_ATTACHMENTS_DIR).",
    )
    parser.add_argument(
        "static_images_dir",
        nargs="?",
        default=None,
        help="Hugo static images directory (overrides BLOG_STATIC_IMAGES_DIR).",
    )
    args = parser.parse_args()

    try:
        # Positional arguments take precedence over environment defaults so
        # shell callers control which blog tree is processed.
        config = None
        overrides = {
            "posts_dir": args.posts_dir,
            "attachments_dir": args.attachments_dir,
            "static_images_dir": args.static_images_dir,
        }
        if any(value for value in overrides.values()):
            config = ImagesProcessor._load_configuration(None)
            for key, value in overrides.items():
                if value:
                    config[key] = value

        processor = ImagesProcessor(config)
