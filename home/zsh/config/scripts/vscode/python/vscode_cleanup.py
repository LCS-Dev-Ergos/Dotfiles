...
def _cleanup_quarantine_root(root: Path) -> tuple[Path, Path]:
    ...
    root_fragment = (
        str(root).replace(str(Path.home()), "HOME").strip("/").replace("/", "__")
    )
...
def apply_cleanup_plan(plan: CleanupPlan) -> CleanupApplyReport:
    ...
    for path in deletable_paths:
        ...
        if not path.exists():
            continue
        if path.is_symlink():
            # The plan only selects real directories; a symlink here means
            # the entry changed after planning, so refuse to move it.
            failed_paths.append(path)
            continue
        if not path.is_dir():
            failed_paths.append(path)
