...
from vscode_sync_apply import ExtensionMutationError, apply_extension_setup
...
def _shared_update_line_reporter(
...
def _unique_backup_target(backup_root: Path, path: Path) -> Path:
    ...
    raise RuntimeError(
        "failed to allocate a unique native excluded update backup target"
    )
    ...
    try:
        ...
        for source_path in paths_to_backup:
            if not is_path_location_within_directory(source_path, insiders_dir):
                raise RuntimeError(
                    "refusing to back up a path outside the Insiders root"
                )
    ...
    with _make_native_update_temp_root(home) as temp_dir:
        ...
        staged_installs = tuple(
            ...
            for install in scan_extension_root(
                temp_root, edition=VscodeEdition.INSIDERS
            )
            if not install.is_symlink and install.extension_id == extension_id
        )
        ...
        if completed.returncode != 0:
            return "failed"
        if not seeded_from_shared and not _is_newer_install(
            staged_install, current_install
        ):
            return "current"
...
def apply_extension_update(
    ...
    *,
    ...
    if shutil.which("code") is None:
        raise ExtensionMutationError("VS Code Stable CLI not found: code")
    if shutil.which("code-insiders") is None:
        raise ExtensionMutationError("VS Code Insiders CLI not found: code-insiders")
    ...
    try:
        ...
        raise ExtensionMutationError(
            f"Shared Stable extension update timed out after {_UPDATE_TIMEOUT_SECONDS}s."
        ) from exc
    shared_update_succeeded = shared_update_completed.returncode == 0
    if not shared_update_succeeded:
        raise ExtensionMutationError("Shared Stable extension update failed.")
    shared_stdout = (
        shared_update_completed.stdout
        if isinstance(shared_update_completed.stdout, str)
        else ""
    )
    shared_stderr = (
        shared_update_completed.stderr
        if isinstance(shared_update_completed.stderr, str)
        else ""
    )
    ...
    for extension_id in plan.native_excluded_extension_ids:
        ...
        if result == "applied":
            ...
            _emit_progress(
                progress, f"Excluded extension already current: {extension_id}."
            )
        else:
            excluded_updates_failed.append(extension_id)
            _emit_progress(
                progress, f"Excluded extension update failed: {extension_id}."
            )
