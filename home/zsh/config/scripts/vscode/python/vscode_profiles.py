...
def _iter_manifest_dict_items(payload: list[object]) -> list[tuple[int, dict]]:
    ...
    return [
        (index, item) for index, item in enumerate(payload) if isinstance(item, dict)
    ]
...
def _build_extension_indexes(
    ...
    for extension_id in list(installs_by_id):
        installs_by_id[extension_id] = _sort_installs_by_preference(
            installs_by_id[extension_id]
        )
...
def _excluded_for_entry(
    ...
    if current_folder_name and is_excluded_extension(
        current_folder_name, exclude_patterns
    ):
        return True
...
def _best_candidate_for_entry(
    *,
    ...
    if extension_id:
        excluded_entry = _excluded_for_entry(
            extension_id, current_folder_name, exclude_patterns
        )
    ...
    if current_folder_name:
        ...
        if (
            ...
            and current_folder_name in insiders_by_name
        ):
...
def _updated_location_payload(
    location: dict[str, object], desired_path: Path
) -> dict[str, object]:
...
def plan_manifest_repairs(
    ...
    *,
    ...
    insiders_installs = scan_extension_root(
        insiders_root, edition=VscodeEdition.INSIDERS
    )
    ...
    manifest_specs = iter_manifest_paths_for_extensions_dir(
        stable_root, config=resolved_config
    )
    manifest_specs += iter_manifest_paths_for_extensions_dir(
        insiders_root, config=resolved_config
    )
    ...
    for manifest_path, source_kind in manifest_specs:
        ...
        for entry_index, item in _iter_manifest_dict_items(manifest_items):
            ...
            current_path = (
                current_root / current_folder_name if current_folder_name else None
            )
...
def build_safe_sync_manifest_plan(plan: ManifestRepairPlan) -> ManifestRepairPlan:
    ...
    keep_count = sum(
        1 for decision in filtered_decisions if decision.action == ManifestAction.KEEP
    )
...
def apply_manifest_repair_plan(plan: ManifestRepairPlan) -> ManifestApplyReport:
    ...
    for manifest_path in sorted(grouped):
        ...
        decisions_by_index = {
            decision.entry_index: decision for decision in grouped[manifest_path]
        }
        ...
        for index, item in enumerate(payload):
            ...
            if _extract_current_folder_name(item) != decision.current_folder_name:
                # The manifest changed between planning and apply; applying a
                # stale decision by index could touch the wrong entry.
                updated_payload.append(item)
                continue
            ...
            if decision.desired_folder_name:
                ...
                desired_path = canonicalize_path(
                    current_root / decision.desired_folder_name
                )
                updated_item["relativeLocation"] = decision.desired_folder_name
                location = updated_item.get("location")
                if not isinstance(location, dict):
                    location = {}
                updated_item["location"] = _updated_location_payload(
                    location, desired_path
                )
...
def _write_manifest_payload_atomically(
    manifest_path: Path, payload: list[object]
) -> None:
    ...
    try:
        ...
        if original_mode is not None:
            # mkstemp creates 0600 files; keep the manifest's existing mode.
...
def apply_manifest_repair_plan_safely(plan: ManifestRepairPlan) -> ManifestApplyReport:
    ...
    for manifest_path, signature_before in profile_signatures_before.items():
        signature_after = _profile_manifest_signature(
            _load_manifest_payload(manifest_path)
        )
